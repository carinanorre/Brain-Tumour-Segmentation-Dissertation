...
# The LGG-UNetModel-Eval file loads any LGG model and evaluates the performance on the train and validate sets
...
# Any LGG model can be read in while applying the custom functions:
modelUNet = tf.keras.models.load_model('./Saved_models/individual-model-LGG.h5',
...
print("Dice scores LGG validation data: ", dice_function_loop(Y_val_reshape, val_Y_pre))

# The following section calculates the confusion matrix for the validation data as well as produces some sample
# segmentation images.
...
plt.title("LGG validation data")
plt.savefig('confusion-matrix-LGG-validation-indiv.png')
...
Y_train_reshape = Y_train.reshape(-1, 192, 192, 1)

# The dice_function_loop is called to evaluate the predictions with the ground truth labels:
print("Dice scores LGG train data: ", dice_function_loop(Y_train_reshape, train_Y_pre))
...
print(confusion_matrix(train_y_train_reshaped, train_y_pre_reshaped))
...
for j in range(4):
    for i in range(4):
        cf_matrix_classpercent[j][i] = \
            cf_matrix_train[j][i] / (cf_matrix_train[j][0]+cf_matrix_train[j][1] +
                                     cf_matrix_train[j][2]+cf_matrix_train[j][3])
        cf_matrix_classpercent[j][i] = '{:.3f}'.format(cf_matrix_classpercent[j][i])
print(cf_matrix_classpercent)

# The confusion matrix is turned into a heatmap and saved to the current directory.
sns.heatmap(cf_matrix_train, annot=True, fmt='.2%', cmap='Blues', cbar=False)
plt.xlabel("Predicted Values")
plt.ylabel("Ground truth values")
plt.title("LGG train data")
plt.savefig('confusion-matrix-LGG-train-indiv.png')

# The following loop takes slices 295-300 and saves their corresponding X data, Y data, and the predicted segmentation.
for i in range(295, 300):
    print('X_train ' + str(i))
    plt.imshow(X_train[i, :, :, 2])
    plt.savefig('X_train ' + str(i))
    plt.show()
    plt.imshow(train_Y_pre[i, :, :, 0])
    plt.savefig('train_Y_pre ' + str(i))
    plt.show()
    plt.imshow(Y_train_reshape[i, :, :, 0])
    plt.savefig('Y_train ' + str(i))
    plt.show()
